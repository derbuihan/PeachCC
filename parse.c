...
static Type *abstract_declarator(Token **rest, Token *tok, Type *ty) {
    ...
    if (equal(tok, "(")) {
        Token *start = tok;
        Type dummy = {};
        abstract_declarator(&tok, tok->next, &dummy);
        tok = skip(tok, ")");
        ty = type_suffix(rest, tok, ty);
...
static Token *skip_excess_elemnt(Token *tok) {
    if (equal(tok, "{")) {
        tok = skip_excess_elemnt(tok->next);
        return skip(tok, "}");
    }
...
static int64_t eval(Node *node) {
    ...
    error_tok(node->tok, "not a compile-time expression");
}
...
static Member *struct_members(Token **rest, Token *tok, Type *ty) {
    Member head = {};
    Member *cur = &head;
